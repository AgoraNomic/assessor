...
fun GeneralVotingStrengthReceiver.powerStone(person: Person, amount: VotingStrengthDifference) {
    person add amount describedAs VotingStrengthModificationDescription(
        readable = "Increased by $amount due to Power Stone",
        kind = "power_stone",
        parameters = mapOf(
...
fun GeneralVotingStrengthReceiver.powerStone(person: Person, amount: Int) =
    powerStone(person, VotingStrengthDifference(amount))
